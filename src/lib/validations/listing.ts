...
export const listingSchema = z.object({
  ...
  priceAmount: z.number({ message: "Price must be a number" }).min(1, "Price must be at least 1 XLM"),
  deliveryTimeframe: z.number({ message: "Required" }).min(1, "Must be at least 1 day").max(30, "Max 30 days"),
  disputePeriod: z.number({ message: "Required" }).min(1, "Must be at least 1 day").max(14, "Max 14 days"),
