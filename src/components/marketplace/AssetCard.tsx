...
export default function AssetCard({ asset, delay = 0 }: { asset: MockAsset; delay?: number }) {
  ...
  return (
    <ScrollReveal delay={delay}>
      <div className="group relative rounded-[2.5rem] bg-white/5 border border-white/10 overflow-hidden hover:border-blue-500/50 transition-all duration-700 hover:shadow-[0_0_50px_-12px_rgba(59,130,246,0.3)] flex flex-col h-full">
        ...
        <div className="p-8 relative z-10 bg-black/40 backdrop-blur-xl flex-grow flex flex-col justify-between">
          <div>
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
              <h3 className="text-lg md:text-xl font-bold text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight line-clamp-2">{asset.name}</h3>
          ...
          <div className="flex items-center justify-between text-xs text-neutral-500 font-bold tracking-wide mt-auto">
            <span className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5"><Clock className="w-3.5 h-3.5" /> {asset.timeStr}</span>
